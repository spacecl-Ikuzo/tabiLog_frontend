...
const TripPlannerPage = () => {
  ...
  const [userRole, setUserRole] = useState<string>('VIEWER'); // 기본값을 VIEWER로 설정
  ...
  const checkUserRole = React.useCallback(
    (planData: {
      ...
      if (!planData?.members || !userId) {
        return 'VIEWER';
      }

      const member = planData.members.find((m) => m.userIdString === userId);
...
<<<<<<< HEAD
        ...
        console.log('=== 사용자 역할 확인 ===', {
          ...
          planMembers: planData?.members,
          userRole: role,
        });

        const hasLocalStorageData = false;
...
<<<<<<< HEAD
  ...
  const addSpot = async (
    ...
    if (currentSpots.length > 0) {
      ...
      const lastSpotEndTime = getLastSpotEndTime(currentSpots, departureTime);
      arrivalTime = {
      ...
      <div className="flex flex-col lg:hidden flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          <MobileSpotList
            ...
            planData={planData}
            userRole={userRole}
            onAddSpot={() => setIsSearchDialogOpen(true)}
            onEditSpot={editSpot}
            onDeleteSpot={deleteSpot}
            onCalculateCost={calculateCost}
            onSetDepartureTime={() => setIsDepartureTimeDialogOpen(true)}
            getEndTime={getEndTime}
          />
        </div>
      </div>
      ...
      <div className="hidden lg:flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-col bg-white" style={{ width: `${leftPanelWidth}%` }}>
          <ItineraryPlanner
            ...
            onEditSpot={editSpot}
            onDeleteSpot={deleteSpot}
            onMoveSpotUp={moveSpotUp}
            onMoveSpotDown={moveSpotDown}
            onCalculateCost={calculateCost}
...
<<<<<<< HEAD
      <EditSpotDialog
        ...
        onUpdateTransportMode={updateSpotTransportMode}
        onUpdateDuration={async (newDuration) => {
          const updatedSpots = (spots[activeDay] || []).map((spot) =>
