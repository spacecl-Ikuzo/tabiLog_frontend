...
import { Plus, MapPin, Trash2, Edit, ArrowLeft, ArrowRight, ChevronUp, ChevronDown, GripVertical, Calculator } from 'lucide-react';
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4 text-sm text-gray-500">
                <div className="flex items-center space-x-1">
                  ...
                  <span>{spot.time} - {getEndTime(spot.time, spot.duration)}</span>
                </div>
                <div className="flex items-center space-x-1">
                  <SvgIcon name="clock" size={16} className="text-gray-500" />
                ...
                {spot.rating && spot.rating > 0 && (
                  <div className="flex items-center space-x-1">
                    <span>⭐</span>
                    <span>{spot.rating.toFixed(1)} ({spot.userRatingsTotal?.toLocaleString()}件のレビュー)</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col items-end space-y-2">
                ...
                {spotExpenses[spot.id] && spotExpenses[spot.id].length > 0 && (
                  <span className="text-sm font-bold text-gray-900">
                    {spotExpenses[spot.id].reduce((sum: number, expense: ExpenseData) => sum + expense.amount, 0).toLocaleString()}円
                  </span>
                )}
...
const TripPlannerPage = () => {
  ...
  // 관광지 추가 함수
  const addSpot = async (newSpot: Spot, duration: string = "1時間", transportMode: 'walking' | 'driving' | 'transit' = 'walking') => {
    ...
    if (currentSpots.length > 0) {
      ...
      // 이동 시간 추가
      let totalMinutes = arrivalTime.hour * 60 + arrivalTime.minute + travelTime;
      arrivalTime = {
  ...
  const calculateTravelTime = async (fromSpot: Spot, toSpot: Spot): Promise<number> => {
    ...
    try {
      ...
      const mode = toSpot.transportMode === 'transit' ? 'TRANSIT' : 
                   toSpot.transportMode === 'driving' ? 'DRIVING' : 'WALKING';
  ...
  const currentSpots = spots[activeDay] || [];
      ...
      if (data.success && data.data) {
        ...
        if (data.data.routes && data.data.routes.length > 0 && 
            data.data.routes[0].legs && data.data.routes[0].legs.length > 0 &&
  ...
  const addTravelSegment = async (fromSpot: Spot, toSpot: Spot) => {
    ...
    const isZeroResults = travelTime === 15 && (!fromSpot.latitude || !fromSpot.longitude || !toSpot.latitude || !toSpot.longitude);
  ...
  const recalculateTravelTimes = async (updatedSpots?: Spot[]) => {
    ...
    for (let i = 1; i < currentSpots.length; i++) {
      ...
      let totalMinutes = prevStartHour * 60 + prevStartMinute + prevDurationMinutes + travelTime;
  ...
  // 관광지 변경 함수
  const changeSpot = async (newSpot: Spot, duration: string = "1時間", transportMode: 'walking' | 'driving' | 'transit' = 'walking') => {
  ...
  const deleteSpot = (day: number, spotId: number) => {
    ...
    setTravelSegments(prev => {
      ...
      for (let i = 1; i < remainingSpots.length; i++) {
        ...
        // 기존 세그먼트에서 찾기
        const existingSegment = prev[day].find(segment => 
          segment.fromSpot?.id === fromSpot.id && segment.toSpot?.id === toSpot.id
        );
        ...
        {currentSpots.length === 0 ? (
          ...
          <div className="space-y-3">
            {currentSpots.map((spot, index) => {
              const nextSegment = currentSegments.find(segment => 
  ...
  const ItineraryPlanner = () => {
    ...
    const tripTotal = Object.values(spots).flat().reduce((total, spot) => {
      const cost = parseInt(spot.cost.replace(/[^\d]/g, '') || '0');
      const expenses = spotExpenses[spot.id] || [];
      const expenseTotal = expenses.reduce((sum, expense) => sum + expense.amount, 0);
      return total + cost + expenseTotal;
...
          </div>
            ...
            >
              <SortableContext
                items={currentSpots.map(spot => spot.id)}
          ...
          <div className="absolute inset-y-0 -left-2 -right-2 flex items-center justify-center">
            <div className={`w-1 h-12 rounded-full transition-all duration-200 ${
              isDragging 
                ...
                : 'bg-gray-400 group-hover:bg-gray-500 group-hover:h-16'
            }`}></div>
          </div>
      ...
      {isCostDialogOpen && costCalculatingSpot && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-[400px] shadow-2xl">
            {/* 제목 */}
            <h3 className="text-xl font-bold text-orange-500 mb-6 text-center">
              費用追加
            </h3>
                  ...
                  >
                    <img
                      ...
                      className={`w-6 h-6 mb-1 ${
                        expenseInputs.category === category.key ? 'brightness-0 invert' : ''
                      }`}
                    />
